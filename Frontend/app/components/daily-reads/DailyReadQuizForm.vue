...
function cancelEdit() {
  // Remove unsaved new questions from quiz array
  if (editingQuestionSeq.value !== null) {
    const editingQuestion = quiz.value.find(q => q.questionSeq === editingQuestionSeq.value)
    if (editingQuestion && !editingQuestion.id) {
      quiz.value = quiz.value.filter(q => q.questionSeq !== editingQuestionSeq.value)
    }
  }
...
function addNewQuestion() {
  ...
  // Add dummy question to quiz array so it renders in the list
  quiz.value.push({
    questionSeq: nextSeq,
    question: '',
    correctAnswer: 'A',
...
async function deleteQuestion(questionSeq: number) {
  ...
  dialog.confirm({
    ...
    message: `Are you sure you want to delete question #${questionSeq}? This action cannot be undone.`,
    async onOk() {
      try {
        await $authedFetch(
...
<template>
  <div class="flex flex-col lg:flex-row gap-6">
    <!-- Actions Sidebar (30%) -->
    <div class="w-full lg:flex-[3] min-w-0">
      <div class="lg:sticky lg:top-6 lg:max-h-[calc(100vh-9rem)] lg:overflow-y-auto space-y-4">
        <UCard>
          <template #header>
          ...
          <div class="space-y-4">
            ...
            <!-- File Upload -->
            <div class="space-y-2">
              <label class="text-xs font-medium text-gray-700 dark:text-gray-300">Upload Quiz File</label>
              <UInput
                type="file"
                accept=".xlsx,.xls"
                size="sm"
              ...
              <p class="text-xs text-gray-500">
                Upload after saving daily read
              </p>
            </div>
          </div>
        </UCard>
    ...
    <div class="w-full lg:flex-[7] min-w-0">
      ...
      >
        ...
        />
        <p class="text-gray-500">
          No quiz questions yet. Click "Add Question" to start.
        </p>
      </div>
      ...
      >
        <div
          ...
          :class="editingQuestionSeq === question.questionSeq ? 'border-primary-500 bg-muted' : ''"
        >
          <!-- Edit Mode -->
          <div
